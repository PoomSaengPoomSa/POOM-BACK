...
try:
    print("--- IN_CHARGE ---")
    in_charges = db.query(InCharge).filter(InCharge.u_id == "pb_b1_1").all()
    print(f"Total in-charges for pb_b1_1: {len(in_charges)}")
    c_ids = [ic.c_id for ic in in_charges]
    print(f"c_ids: {c_ids}")
    
    print("\n--- CUSTOMERS ---")
    customers = db.query(Customer).filter(Customer.c_id.in_(c_ids)).all()
    print(f"Total customer records found for pb_b1_1: {len(customers)}")
    for c in customers:
